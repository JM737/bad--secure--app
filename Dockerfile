...
# FAILS: validator does not enforce minimum length,
# so a 2-character username like "ab" incorrectly returns 200
def test_greet_invalid_username_too_short(client):
    response = client.post('/greet', json={"username": "ab"})
    assert response.status_code == 422  # expects rejection, but app returns 200
...
def test_validator_rejects_short():
    # BUG exposed: returns True for short usernames
    assert is_valid_username("ab") is False  # will FAIL
